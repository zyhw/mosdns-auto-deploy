...
| 配置项 | 说明 |
|--------|------|
| 国内 DNS | 阿里 `tls://223.5.5.5` (开启 Pipeline)、腾讯 `tls://1.12.12.12`、`tls://120.53.53.53`，两路并发 (`concurrent: 2`) 以防被封锁限制 |
| 国外 DNS | Google `tls://8.8.8.8`、CF `tls://1.1.1.1`、OpenDNS `tls://208.67.222.222`，两路并发 |
| ECS 加成 | 国内流量附加本地 IP 掩码，极大提升阿里/腾讯等公共 DNS 返回的国内 CDN 节点精度 |
| 内网防漏 | 强制丢弃本地 PTR 请求 (`qtype 12`)，阻止局域网反向解析泄露到公网 |
| `prefer_ipv4` | 国外解析时 **IPv4 优先** |
| Fallback | 本地 DNS 若返回非国内 IP → 丢弃 → 改用国外 DNS（防污染） |
| 缓存 | 2 万条，持久化到磁盘，lazy TTL 24h |
| 容灾防崩 | 日常规则更新采用 `安全下载 -> 校验非空 -> 覆盖替换` 机制。断网/源站 502 时不会破坏现有系统 |
